...
from fabric.api import lcd, local, prefix, prompt, run, settings, task
...
def build(**options):
    ...
    with settings(hide("stdout", "stderr"), warn_only=True):
        with lcd(PYPY):
            local(
                "python setup-pypy.py {0:s}".format(
                    "develop" if dev else "install"
                )
            )

            local(
                "python setup-rpython.py {0:s}".format(
                    "develop" if dev else "install"
                )
            )
...
def sync(*args):
    ...
    if status:
        abort("Repository is not in a clean state!")
